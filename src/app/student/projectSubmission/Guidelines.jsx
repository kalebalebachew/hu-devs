...
import { ArrowRight, CheckCircle, FileCode, Globe2, BookOpen, FileText, Info } from 'lucide-react';
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="bg-primary/10 text-primary hover:bg-primary/20 hover:text-primary">
          View Guidelines
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {guidelines.map((section, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="border rounded-lg px-4 data-[state=open]:bg-muted transition-colors"
              >
